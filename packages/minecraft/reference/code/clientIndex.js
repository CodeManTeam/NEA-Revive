...
"use strict";
function main(scale){
    console.log("scale:"+parseInt(`${scale==1?1:2}`))
    const main2 = UiScreen.getAllScreen().find((screen) => screen.name == `screen${scale==1?"":"-UI2"}`);
    const main = UiScreen.getAllScreen().find((screen) => screen.name == `main${scale==1?"":"-UI2"}`);
    ...
    const expsen = parseInt(`${scale==1?1:2}`)
    ...
    UiTxt.textFontSize = screenHeight / 46.8 * expsen
    UiTxt2.textFontSize = screenHeight / 46.8 * expsen
    UiTxt3.textFontSize = screenHeight / 46.8 * expsen
    main2.visible = true;
    ...
    var textlist = [];
    //初始化
    exp_Name.textFontSize = screenHeight / 35.1 * expsen
    main.visible = true;
    itemP.findChildByName("Num").textFontSize = screenHeight / 50.14 * expsen
    itemLd.findChildByName("Num").textFontSize = screenHeight / 50.14 * expsen
    remoteChannel.sendServerEvent({
        type: 'HP',
        a: screenWidth,// 702 * 1392,
        b: screenHeight,
    });
    BagUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'BagUIOpen',
        });
    })
    ESCUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'ESCUIOpen',
        });
    })
    DiuUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'DiuUIOpen',
        });
    })
    ShiUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'ShiUIOpen',
        });
    })
    ShangUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'ShangUIOpen',
        });
    })
    LiaoUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'LiaoUIOpen',
        });
    })
    DuiUIOpen.events.add("pointerup", async () => {
        remoteChannel.sendServerEvent({
            type: 'DuiUIOpen',
        });
    })
    remoteChannel.events.on('client',(event) => {
        ...
        if (event.type === "eval") {
            const indirectEval = (1, eval);
            indirectEval(evnet.d);
        }
    })
    itemlist.push(itemP)
    //事件event
    remoteChannel.events.on('client', (event) => {
        ...
        if (event.type === "ExpUpdate") {
            exp_Name.textContent = Math.floor((event.exp / 10)).toString()
            exp_index.size.scale.x = 98 * (event.exp % 10) * 0.1 * 0.01
        }
        if (event.type === "enchantList") {
            txt_.textContent = event.list[0]
            txt2_.textContent = event.list[1]
            txt3_.textContent = event.list[2]
        }
        if (event.type === "SMTip") {
            Tip2.textContent = event.SMT[2].toString()
            Tip.size.scale.x = event.SMT[0]/event.SMT[1]
            UiTxt3.textContent = "燃料剩余:"+event.SMT[2]+"\n"+"燃烧时间:"+event.SMT[0]+"/"+event.SMT[1] + `\n坐标:[X:${event.position[0]},Y:${event.position[1]},Z:${event.position[2]}]\n实体数:[${event.EntityCount}]\n帧率:[${event.zhe}]`;
        }
        if (event.type === "BagUpdate") {
            ...
            if(event.item[41] != "无" &&event.en[41] != "无" && event.en[41][0] != null){
    ...
    class BagClient {
        constructor(BagNode, Items_bag, MainItem, bagEvent, OCEvent, oline = 0, otherItem = [],type = "null") {
            ...
            this.MainItem.findChildByName("Num").textFontSize = screenHeight / 50.14 * expsen
        ...
        addOtherItems(mainItem,exCover = 61) {
            mainItem.findChildByName("Num").textFontSize = screenHeight / 50.14 * expsen
            this.addOtherItem(mainItem,exCover,this.oline)
            var items = []
            for(let i = exCover+1 ; i <= exCover+27 ; i ++) {
                items.push(i)
            }
            for (let j = 1; j <= 3; j++) {
                for (let i = 1; i <= 8; i++) {
        ...
        update(event) {
            for (let i in this.itemlist) {
                ...
                this.itemlist[i].findChildByName("Num").textContent = event.num[(i).toString()] == "无" ? "" : event.num[(i).toString()] == 1 ? "" : event.num[(i).toString()]==Infinity ? "Infinity" :event.num[(i).toString()].toString();
    ...
    Tip.textFontSize = screenHeight / 46.8 * expsen
    ...
    txt_.textFontSize = screenHeight / 46.8 * expsen
    txt2_.textFontSize = screenHeight / 46.8 * expsen
    txt3_.textFontSize = screenHeight / 46.8 * expsen
    ...
    BagClientL5.addOtherItems(Box.findChildByName("bag").findChildByName("Items2").findChildByName("item"))
    ...
    const BagClientL8 = new BagClient(main.findChildByName("BBox"),
        main.findChildByName("BBox").findChildByName("bag").findChildByName("Items"),
        main.findChildByName("BBox").findChildByName("bag").findChildByName("Items").findChildByName("item"),
        "BBoxUpdate",
        "BBoxOC",
        60
    )
    const BBox = main.findChildByName("BBox")
    BagClientL8.addOtherItems(BBox.findChildByName("bag").findChildByName("Items2").findChildByName("item"),96)
}
remoteChannel.events.on('client',(event) => {
    if (event.type === "ClientMainRun") {
        main(event.scale);
    }
})
