...
#[derive(Default)]
pub struct HistoricalUi {
    pub state: GameUiState,
    pub layout: BTreeMap<String, UiRect>,
}
...
impl HistoricalUi {
    pub fn replace(&mut self, state: GameUiState, viewport: [f32; 2]) {
        ...
        let roots: Vec<String> = self
            ...
            .nodes
            .values()
            .filter(|n| n.parent_id.is_empty() || !self.state.nodes.contains_key(&n.parent_id))
            .map(|n| n.id.clone())
            .collect();
        for id in roots {
            self.layout_node(&id, None, viewport);
        }
    }
    fn layout_node(&mut self, id: &str, parent: Option<UiRect>, viewport: [f32; 2]) {
        let Some(node) = self.state.nodes.get(id).cloned() else {
            return;
        };
        let rect = match node.value.as_ref() {
            Some(UiNodeValue::Element(element)) => {
                let common = match element {
                    ...
                    | UiElement::Image { common, .. }
                    | UiElement::Text { common, .. }
                    | UiElement::Input { common, .. }
                    | UiElement::ScrollBox { common, .. } => common,
                };
                let base = parent.unwrap_or(UiRect {
                    x: 0.0,
                    y: 0.0,
                    width: viewport[0],
                    height: viewport[1],
                ...
                let x = base.x
                    + common.position.offset[0]
                    + base.width * common.position.ratio[0]
                    + base.width * common.anchor[0];
                let y = base.y
                    + common.position.offset[1]
                    + base.height * common.position.ratio[1]
                    + base.height * common.anchor[1];
                UiRect {
                    x,
                    y,
                    width: common.size.offset[0] + base.width * common.size.ratio[0],
                    height: common.size.offset[1] + base.height * common.size.ratio[1],
                    z: common.z_index,
                }
            }
            Some(UiNodeValue::Screen(screen)) => UiRect {
                x: 0.0,
                y: 0.0,
                width: viewport[0],
                height: viewport[1],
        ...
        for child in node.children_ids {
            self.layout_node(&child, Some(rect.clone()), viewport);
        }
    }
    ...
    pub fn render_dom(&self, document: &Document) -> Result<(), JsValue> {
        ...
        for (id, node) in &self.state.nodes {
            ...
            let Some(UiNodeValue::Element(element)) = node.value.as_ref() else {
                continue;
            };
            let dom = match element {
                ...
                UiElement::Image {
                    ...
                    let url = self
                        ...
                        .picture_assets
                        .get(image)
                        .map(|asset| format!("/engine/m/{}", asset.hash))
                        .unwrap_or_default();
                    if !url.is_empty() {
                        e.set_attribute("src", &url)?;
                    }
                    let fit = match image_display_mode {
                        1 => "contain",
                        2 => "cover",
                        _ => "fill",
                    };
            ...
            if !matches!(
                element,
                UiElement::Input { .. } | UiElement::ScrollBox { .. }
            ) {
...
fn attach_ui_event(element: &Element, node_id: &str, kind: &str) -> Result<(), JsValue> {
    ...
    let closure = Closure::wrap(Box::new(move |event: web_sys::Event| {
        ...
        {
            let value = js_sys::Reflect::get(&input, &JsValue::from_str("value"))
                .unwrap_or(JsValue::UNDEFINED);
            let _ = js_sys::Reflect::set(&detail, &JsValue::from_str("value"), &value);
            let scroll_top = js_sys::Reflect::get(&input, &JsValue::from_str("scrollTop"))
                .unwrap_or(JsValue::UNDEFINED);
            let scroll_left = js_sys::Reflect::get(&input, &JsValue::from_str("scrollLeft"))
                .unwrap_or(JsValue::UNDEFINED);
        ...
        if let Some(window) = web_sys::window() {
            let _ = js_sys::Reflect::set(
                &window,
                &JsValue::from_str("__NEA_HISTORICAL_UI_EVENT"),
                &detail,
            );
...
mod tests {
    ...
    fn layout_resolves_offset_and_ratio() {
        ...
        state.nodes.insert(
            "root".into(),
            voxweb_protocol::UiNode {
                id: "root".into(),
                children_ids: vec!["box".into()],
                ..Default::default()
            },
        );
        state.nodes.insert(
            ...
            voxweb_protocol::UiNode {
                id: "box".into(),
                parent_id: "root".into(),
                value: Some(UiNodeValue::Element(UiElement::Box {
                    common,
                    rotation: 0.0,
                })),
                ..Default::default()
        ...
        let mut ui = HistoricalUi::default();
        ui.replace(state, [800.0, 600.0]);
        assert_eq!(ui.layout["box"].x, 210.0);
        assert_eq!(ui.layout["box"].y, 320.0);
    }
}
