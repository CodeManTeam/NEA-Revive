...
pub mod interp;
pub mod mesh_jobs;
mod nea_chat_overlay;
mod nea_client_runtime;
mod nea_damage_overlay;
mod nea_eye_ambient;
...
mod nea_prediction;
mod nea_session;
mod nea_session_context;
mod nea_session_runtime;
mod nea_session_transport;
mod nea_smoke;
mod nea_voxel_light;
pub mod physics;
pub mod prediction;
...
pub async fn start() -> Result<(), JsValue> {
    ...
    // NEA session smoke path (?nea=<createSessionUrl>) — console-only,
    // does not touch the default game canvas/renderer. VW-003 Step 1.
    if let Some(nea_url) = read_query_param("nea")
        && !nea_url.is_empty()
    {
        return crate::nea_smoke::run(&nea_url).await;
    }
