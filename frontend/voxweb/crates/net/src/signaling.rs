...
impl SignalingClient {
    ...
    pub fn connect(
        ...
        {
            let inbox_clone = inbox.clone();
            let on_error = Closure::<dyn FnMut(JsValue)>::new(move |_evt: JsValue| {
                // Browser WebSocket failures dispatch a generic Event in Chromium.
                // Reading ErrorEvent.message from that value yields undefined and
                // makes wasm-bindgen's generated string bridge panic.
                let message = "WebSocket signaling connection failed".to_owned();
                inbox_clone
                    .borrow_mut()
